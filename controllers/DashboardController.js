...
class DashboardController {
    ...
    admin = (req, res) => {
        has_user_logged_in(req).then(user => {
            if (user) {
                ...
                if (user.is_admin) {
                    this.Product.fetch_all_by_user_id(user.id)
                        .then(products => {
                            // normal user dashboard
	...
	*/
    process_edit_user_info = (req, res) => {
        ...
        this.User.edit_user_info_by_id(fields, req.session.user)
            .then(result => {
    ...
    process_edit_user_password = (req, res) => {
        ...
        if (fields.new_password === fields.confirm_new_password) {
            this.User.edit_user_password_by_id(fields, req.session.user)
                .then(result => {
